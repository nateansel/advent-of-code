...
enum Instruction: Equatable, Hashable {
    ...
    init(rawValue: String) {
        ...
        if components[0] == "noop" {
            self = .noop
        } else if components[0] == "addx", let value = Int(components[1]) {
            self = .add(x: value)
        } else {
            fatalError("Not a valid instruction: \(rawValue)")
        }
    }
}
...
var instructionsToProcess: [Int: Instruction] = [:]
var register = 1
...
let signalStrengthTotal = signalStrengths.reduce(0, +)

print("""
Part 1 Solution:
    Signal Strength Total: \(signalStrengthTotal)
""")
...
var screen = Array(repeating: Array(repeating: false, count: 40), count: 6)

var pixel = 0 {
    willSet {
        let spriteRange = (spritePosition - 1)...(spritePosition + 1)
        let localPixel = pixel % 40
        if spriteRange.contains(localPixel) {
            screen[pixel / 40][localPixel] = true
        }
    }
}
...
let screenLines = screen.map({ $0.map({ $0 ? "#" : "." }).joined() })

print("""
Part 2 Solution:
    \(screenLines.joined(separator: "\n    "))
""")
